        ...
        <div className="flex flex-wrap items-center justify-end gap-x-4 gap-y-3">
          {balances && (
            <div className="flex flex-wrap items-center justify-end gap-x-3 gap-y-1 order-2 md:order-1">
              <div className="flex items-center space-x-2 bg-gray-900/50 px-3 py-1 rounded-full border border-white/10">
                ...
                <span className="text-xs text-gray-400 font-medium">{nativeCurrencySymbol}</span>
              </div>
              <div className="flex items-center space-x-2 bg-gray-900/50 px-3 py-1 rounded-full border border-white/10">
          ...
          <div className="flex items-center space-x-2 order-1 md:order-2">
            <div className="relative">
              <select
                value={networkName}
                onChange={(e) => setCurrentNetwork(e.target.value as NetworkName)}
              ...
              >
                {Object.entries(availableNetworks).map(([key, value]: [string, any]) => (
                  <option key={key} value={key} className="bg-gray-800 text-white">{value.chainName}</option>
                ))}
              </select>
